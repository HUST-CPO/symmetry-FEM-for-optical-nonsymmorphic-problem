function [copyOfBedge,copyOfBvertex,EdgePhi,NodePhi]=GetcopyOfBedge3(PBCEdge1,PBCEdge2,Mesh,ns,tage)
...
for i = 1:size(PBCEdge1,1)
    for j = 1:size(PBCEdge2,1)
       ...
       if(all(all(abs(dif(:,cols)-dif0)<0.000000001)))
            ...
            NodePhi(i,:) = Getglidephi(ns.v,tage);
            NodePhi(size(PBCEdge1,1)+i,:) = Getglidephi(ns.v,tage);
        end
