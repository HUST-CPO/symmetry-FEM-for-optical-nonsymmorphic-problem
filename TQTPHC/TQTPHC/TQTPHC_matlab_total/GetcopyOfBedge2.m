function [copyOfBvertex,NodePhi]=GetcopyOfBedge2(PBCEdge1,PBCEdge2,Mesh,ns)
...
for i = 1:size(PBCEdge1,1)
    for j = 1:size(PBCEdge2,1)
        ...
        if(all(all(abs(dif(:,cols)-dif0)<0.0001)))
            ...
            NodePhi(i,:) = Getphi(1,0,0,ns.u,ns.v,ns.w);
            NodePhi(size(PBCEdge1,1)+i,:) = Getphi(1,0,0,ns.u,ns.v,ns.w);
        end
