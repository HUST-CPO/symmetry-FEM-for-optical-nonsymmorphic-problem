...
for i = 1:size(PBCEdge1,1)
    for j = 1:size(PBCEdge2,1)
        dif = Mesh.Nodes(Mesh.Edges(PBCEdge1(i),:),2)-Mesh.Nodes(Mesh.Edges(PBCEdge2(j),:),1);
       if(all(all(abs(dif(:,:)-dif0)<0.000000001)))
        ...
        dif = Mesh.Nodes(Mesh.Edges(PBCEdge1(i),:),2)-flipud(Mesh.Nodes(Mesh.Edges(PBCEdge2(j),:),1));
        if(all(all(abs(dif(:,:)-dif0)<0.00000001)))
            copyOfBedge(i,1)=PBCEdge1(i);
            copyOfBedge(i,2)=PBCEdge2(j);
            copyOfBvertex(i,1)=Mesh.Edges(PBCEdge1(i),1);
